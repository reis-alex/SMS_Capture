...
for k = 1:20000
    ...
    if k<kcapt
        ...
        [xsat_plus, xtarget_plus] = freefloating_is_captured(dt, xsat(:,k), R0s(:,:,k), x_target(:,k), R0t(:,:,k), satelite, robot_pre_sim, target, zeros(3,1), hcapt);
        xsat(:,k+1) = xsat_plus;
        ...
        Hsv = satelite.dynamics.H(R0s(:,:,k),xsat(:,k));
        Htv = target.dynamics.H(R0t(:,:,k),x_target(:,k));
        total_moment = horzcat(total_moment, compute_moment(satelite, target, R0s(:,:,k),R0t(:,:,k),xsat(:,k),x_target(:,k)));
