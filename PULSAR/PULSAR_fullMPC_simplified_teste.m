...
x_hsat = casadi.SX.sym('hsat',3,1);
x_hwh = casadi.SX.sym('hwheel',3,1);
x_harm = casadi.SX.sym('harm',3,1);
% Define MPC problem
opt.model.states   = [satelite.state_vars([1:14 21:end]); x_hsat; x_hwh; x_harm];
...
texp = [satelite.state_vars(satelite.idx.velocities);f_mpc(7:end);hsat(1:3);hwheel(1:3);harm(1:3)];
...
opt.model.function = casadi.Function('fmpc',{vertcat(satelite.state_vars([1:14 21:28]),x_hsat,x_hwh,x_harm),tau},{texp});
...
opt.parameters.name{4} = 'itm_moment';
opt.parameters.name{5} = 'sign';
opt.parameters.dim = vertcat([3,1], [3,1], [1,1], [3,1], [3,1]);
...
opt.costs.stage.parameters = opt.parameters.name(1:5);
% opt.costs.stage.function   = @(x,u,varargin) sum((ee_fun(x(1:6+satelite.robot.n_q))-varargin{:}(1:3)).^2) + 0*(1-varargin{:}(7))*u(1:3)'*u(1:3) + varargin{:}(7)*( x(18:22)'*x(18:22) + (x(15:17)-[1*x(2);10*x(1);10*x(3)])'*(x(15:17)-[1*x(2);10*x(1);10*x(3)])    )*1e2;
% opt.costs.stage.function   = @(x,u,varargin) sum((ee_fun(x(1:6+satelite.robot.n_q))-varargin{:}(1:3)).^2) + (1-varargin{:}(7))*u(1:3)'*u(1:3) + varargin{:}(7)*( x(18:22)'*x(18:22) + (hsatf(x(1:22))-[0.5;0.5;0.5])'*(hsatf(x(1:22))-[0.5;0.5;0.5])    )*1e2;
opt.costs.stage.function   = @(x,u,varargin) sum((ee_fun(x(1:6+satelite.robot.n_q))-varargin{:}(4:6)).^2) + varargin{:}(7)*( x(18:22)'*x(18:22) + 1e2*harmf(x(1:22))'*harmf(x(1:22)) + (hsatf(x(1:22))-varargin{:}(11:13).*[0.5;0.5;0.5])'*(hsatf(x(1:22))-varargin{:}(11:13).*[0.5;0.5;0.5])    )*1e2;
% opt.costs.stage.function   = @(x,u,varargin) sum((ee_fun(x(1:6+satelite.robot.n_q))-varargin{:}(4:6)).^2) + varargin{:}(7)*( x(18:22)'*x(18:22) + 1e2*harmf(x(1:22))'*harmf(x(1:22)) + (hsatf(x(1:22))-varargin{:}(11:13).*varargin{:}(8:10))'*(hsatf(x(1:22))-varargin{:}(11:13).*varargin{:}(8:10))  - varargin{:}(8:10)'*varargin{:}(8:10)   )*1e2;
...
opt.constraints.states.upper  = vertcat( inf*ones(3,1),  inf*ones(3,1), 3600*2*pi/60*ones(3,1),  inf*ones(satelite.robot.n_q-3,1),  inf*ones(3,1),  0.09*ones(satelite.robot.n_q-3,1), inf*ones(9,1));
opt.constraints.states.lower  = vertcat(-inf*ones(3,1), -inf*ones(3,1), -3600*2*pi/60*ones(3,1), -inf*ones(satelite.robot.n_q-3,1), -inf*ones(3,1), -0.09*ones(satelite.robot.n_q-3,1), -inf*ones(9,1));
...
opt.constraints.general.parameters  = opt.parameters.name([2 4 5]);
...
% opt.constraints.general.function{2} = @(x,varargin) -(varargin{:}(7:9).*varargin{:}(4:6));
% opt.constraints.general.elements{2} = 'end';
% opt.constraints.general.type{2} = 'inequality';

opt.constraints.parameters.name  = opt.parameters.name([2 4]);
opt.constraints.parameters.upper =  vertcat(inf*ones(3,1),ones(3,1));
opt.constraints.parameters.lower = -vertcat(inf*ones(3,1),0*ones(3,1));

% Define inputs to optimization
opt.input.vector = opt.parameters.name([1 3 5]);
opt.solver = 'ipopt';
...
xsat(:,1) = vertcat(xsat_ini([1:14 21:end],1),zeros(9,1));
sw = 0;
sw2 = 1;
feval = zeros(31,1);
...
%     if  norm(mom_arm(:,end))>=0.005 && sw == 0
        htg = zeros(3,1);
        auxi = 0;
        sig = zeros(3,1);
    else
...
%     check_feas(solver_mpc.stats())
    ...
    itmt(:,k) = full(sol.x(end-5:end-3)); 
            sig_t1(:,k) = vertcat(sign(xsat(1,k)),-sign(xsat(2,k)),-sign(xsat(3,k)));
            sig_t2(:,k) = vertcat(sign(feval(23)),-sign(feval(24)),-sign(feval(25)));
