...
% 
%     if isfield(opt.model,'perturbation')
%         opt.model.perturbation = opt.model.perturbation;
%     else
%         opt.model.perturbation = [];
%     end
% 
%     if isfield(opt.model,'parameters')
...
U = [];
for i = 1:opt.n_controls
    U = vertcat(U, SX.sym(name(opt.model.controls(i)),1,opt.N));
end
...
X = [];
for i=1:opt.n_states
    X = vertcat(X, SX.sym(name(opt.model.states(i)),1,opt.N));
end
...
if isfield(opt,'misc') && cell2mat(strfind(opt.misc,'optimize_ic'))
...
args.vars{2} = Param;
args.vars{3} = find(strcmp(names,[name(opt.model.controls(1)) '_0'])):find(strcmp(names,[name(opt.model.controls(1)) '_0']))+opt.n_controls-1;
