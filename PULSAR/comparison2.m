...
opt.costs.stage.function   = @(x,u,varargin) sum((ee_fun(x(1:6+satelite.robot.n_q))-varargin{:}(1:3)).^2) ;

opt.costs.general.parameters = opt.parameters.name(1:3);
opt.costs.general.function   = @(x,u,varargin) 1e4*(varargin{2}-varargin{1})'*(varargin{2}-varargin{1}) + (x(1:3,end)'*1e3*x(1:3,end))/(1-varargin{3}+eps); %+ omega0(x(:,end))'*1e2*omega0(x)

opt.constraints.states.upper  = vertcat( p*ones(3,1),  inf*ones(3,1), inf*ones(3,1),  inf*ones(satelite.robot.n_q-3,1),  3600*2*pi/60*ones(3,1),  0.09*ones(satelite.robot.n_q-3,1));
opt.constraints.states.lower  = vertcat(-p*ones(3,1), -inf*ones(3,1), -inf*ones(3,1), -inf*ones(satelite.robot.n_q-3,1), -3600*2*pi/60*ones(3,1), -0.09*ones(satelite.robot.n_q-3,1));
...
opt.constraints.general.function{3} = @(x,u,varargin) vertcat(harmf(x)-100, -harmf(x)-100);
opt.constraints.general.elements{3} = 'end';
...
for k = 1:6000
    ...
    aux = min(1,xsat(1:3,k)'*P*xsat(1:3,k));
