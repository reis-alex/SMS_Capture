...
p = 10*pi/180;
P = eye(3)*1/(p^2);
opt.costs.stage.parameters = opt.parameters.name([2 3]);
% opt.costs.stage.function   = @(x,u,varargin) sum((ee_fun(x(1:6+satelite.robot.n_q))-varargin{:}(1:3)).^2) + 1e5*(x(1:3)'*x(1:3) + omega0(x)'*omega0(x) + x(18:22)'*x(18:22))/(1-varargin{:}(4:6)'*P*varargin{:}(4:6));
opt.costs.stage.function   = @(x,u,varargin) sum((ee_fun(x(1:6+satelite.robot.n_q))-varargin{:}(1:3)).^2) + varargin{:}(4)*1e5*(x(1:3)'*x(1:3) + omega0(x)'*omega0(x) + u(1:3)'*(1e-7)*u(1:3) + u(4:end)'*(1e1)*u(4:end));

opt.costs.general.parameters = opt.parameters.name([1 2]);
opt.costs.general.function   = @(x,u,varargin) (varargin{2}-varargin{1})'*(varargin{2}-varargin{1});% + (x(1:3,end)'*1e3*x(1:3,end))/(1-varargin{3}+eps); %+ omega0(x(:,end))'*1e2*omega0(x)

opt.constraints.states.upper  = vertcat( p*ones(3,1),  inf*ones(3,1), inf*ones(3,1),  inf*ones(satelite.robot.n_q-3,1),  40*ones(3,1),  0.09*ones(satelite.robot.n_q-3,1));
opt.constraints.states.lower  = vertcat(-p*ones(3,1), -inf*ones(3,1), -inf*ones(3,1), -inf*ones(satelite.robot.n_q-3,1), -40*ones(3,1), -0.09*ones(satelite.robot.n_q-3,1));
...
opt.constraints.general.function{2} = @(x,u,varargin) vertcat(reshape(diff(u(4:end,:)')',5*(opt.N-1),1)-5*ones(5*(opt.N-1),1), ...
                                                              -reshape(diff(u(4:end,:)')',5*(opt.N-1),1)-5*ones(5*(opt.N-1),1)); 
opt.constraints.general.elements{2} = 'all';
opt.constraints.general.type{2} = 'inequality';

% opt.constraints.general.function{3} = @(x,varargin) reshape(vertcat(harmf(x)-100*ones(3,1), -harmf(x)-100*ones(3,1)),30,1);
% opt.constraints.general.elements{3} = 'all';
% opt.constraints.general.type{3} = 'inequality';

% opt.constraints.general.function{3} = @(x,u,varargin) vertcat(reshape(diff(u(1:3,:)')',3*(opt.N-1),1)-0.005*ones(3*(opt.N-1),1), ...
%                                                               -reshape(diff(u(1:3,:)')',3*(opt.N-1),1)-0.005*ones(3*(opt.N-1),1)); 
% opt.constraints.general.elements{3} = 'all';
% opt.constraints.general.type{3} = 'inequality';
...
for k = 1:10000
    ...
    if k<500 %norm(link_positions(:,end,k)-ref)>=0.002 && sw == 0
        aux = 0;
    else
        if sw2 == 1
            aux = 1;
    ...
    mpc_input = vertcat(xsat(:,k),ref,aux);
...
plot(itmt(1,1:k),'-.r')
hold on
plot(ref(1)*ones(1,length(squeeze(link_positions(1,end,1:k)))),'--r')
plot(squeeze(link_positions(1,end,1:k)),'-r')
plot(itmt(2,1:k),'-.g')
plot(ref(2)*ones(1,length(squeeze(link_positions(1,end,1:k)))),'--g')
plot(squeeze(link_positions(2,end,1:k)),'-g')
plot(itmt(3),'-.b')
plot(ref(3)*ones(1,length(squeeze(link_positions(1,end,1:k)))),'--b')
plot(squeeze(link_positions(3,end,1:k)),'-b')
xlabel('Time [s]')
ylabel('Position [m]')
grid on
...
figure
stairs(torque(4,1:k),'r')
hold on
stairs(torque(5,1:k),'g')
stairs(torque(6,1:k),'b')
stairs(torque(7,1:k),'c')
stairs(torque(8,1:k),'m')
xlabel('Time [s]')
ylabel('Torque [Nm]')
grid on
...
figure
subplot(131)
plot(mom_sate(1,1:k),'b'); hold on; plot(mom_arm(1,1:k),'-g'),plot(mom_wheels(1,1:k),'--r')
subplot(132)
plot(mom_sate(2,1:k),'b'); hold on; plot(mom_arm(2,1:k),'-g'),plot(mom_wheels(2,1:k),'--r')
subplot(133)
plot(mom_sate(3,1:k),'b'); hold on; plot(mom_arm(3,1:k),'-g'),plot(mom_wheels(3,1:k),'--r')
legend('mom sat','mom bras', 'mom roues')

figure
plot(mom_wheels(1,1:k),'-r')
hold on
plot(mom_wheels(2,1:k),'-g')
plot(mom_wheels(3,1:k),'-b')
