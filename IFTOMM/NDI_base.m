...
for k = 1:20000
    ...
    % simulate free robot
    vr = -vertcat(K1*vertcat(xsat(1:3,k)-[0;0;0],xsat(13:15,k)-[0;0;0]),zeros(3,1));
    torque_wheels(:,k) = vertcat(full(NDI_wheels(R0s(:,:,k),xsat(:,k),vr)), zeros(3,1));

    vr = -vertcat(zeros(3,1), K1*vertcat(xsat(satelite.idx.q(4:6),k)-[1.57;-0.1569;0.3],xsat(satelite.idx.qdot(4:6),k)-[0;0;0]));
